...
SDL_Rect T_SPRITE_LOCATION = { 9, 49, 48, 32 };


const int SCREEN_WIDTH = 640; //This it the dimension of the board. this is a hack for now.
...
int main(int argc, char* argv[]) {
	...
	else
	{
		...
		Texture spriteSheet;
		boardTexture.loadFromFile(gRenderer, "Tetris_Board.png");
		spriteSheet.loadFromFile(gRenderer, "Tetris_Sprite_Sheet.png");
		SDL_Event event;
		...
		while (game.state != GameState::OVER) {
			...
			if (frameTimeDiff > FRAME_RATE) {
				...
				boardTexture.render(gRenderer, (SCREEN_WIDTH - boardTexture.getWidth()) / 2, 0, 0);
				
				Tetranimo* pieceToDraw = &game.activePiece;
				spriteSheet.render(gRenderer, pieceToDraw->points[0].x * 50 , pieceToDraw->points[0].y * 50, &game.activePiece.spritePos);
...
Tetranimo movePiece(Tetranimo piece, PieceDirection direction)
{
	...
	movedPiece.spritePos = piece.spritePos;
	return movedPiece;
}
...
Tetranimo spawnTetranimo() {
	...
	tetranimo.spritePos = T_SPRITE_LOCATION;
